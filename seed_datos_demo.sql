-- =========================================================
-- DATOS FICTICIOS PARA DEMO DE LUMIERE
-- Correr esto DESPUES de crear la estructura de tablas (schema-only)
-- en la base de datos NUEVA (la de staging/demo), nunca en la real.
-- =========================================================
...
-- Configuracion general del negocio (sin certificado ARCA -- la demo no factura de verdad)
...
-- Usuarios (password: demo123, hasheada con bcrypt)
-- Hash de ejemplo para "demo123" -- si tu version de bcrypt difiere, mejor crear los usuarios
-- con el endpoint /auth/register en vez de este INSERT directo.
INSERT INTO usuarios (id, nombre, email, password, rol, rol_id, local_id) VALUES
  ...
  (2, 'Vendedora Demo 1', 'vendedora1@demo.com', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy', 'vendedora', 3, 1),
  (3, 'Vendedora Demo 2', 'vendedora2@demo.com', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy', 'vendedora', 3, 2)
ON CONFLICT (id) DO NOTHING;
...
-- Proveedores
INSERT INTO proveedores (id, nombre, cuit, condicion_pago, medio_pago, contacto) VALUES
  (1, 'Distribuidora Central', '30712345671', '30 dias', 'transferencia', '5491100000001'),
  (2, 'Mayorista del Sur', '30712345672', '45 dias', 'echeck', '5491100000002'),
  (3, 'Importadora Norte', '30712345673', '15 dias', 'transferencia', '5491100000003')
ON CONFLICT (id) DO NOTHING;

-- Productos (rubro generico -- kiosco / bazar / regaleria, sirve de ejemplo para cualquier retail)
...
-- Ventas de ejemplo, repartidas en los ultimos 30 dias entre los dos locales.
-- generate_series crea una venta por dia (variando un poco el producto/monto).
DO $$
DECLARE
  d INT;
  v_id INT;
...
BEGIN
  FOR d IN 0..29 LOOP
    ...
    prod := 1 + (d % 20);
    cant := 1 + (d % 4);
    SELECT precio INTO precio FROM productos WHERE id = prod;
    ...
    VALUES (
      loc, usr, precio * cant, 'presencial',
